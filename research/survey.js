...
function reasonFields(q){const a=payload.answers[q.id];return `<section class="card"><h2>${e(q.title)}</h2>${q.options.map((o,i)=>a.values.includes(o)&&q.followups?.includes(o)?`<label for="${q.id}-reason-${i}">${e(followupPrompt(q.id,i,language))}</label><textarea class="notes" id="${q.id}-reason-${i}" maxlength="1000">${e(a.reasons?.[o]||'')}</textarea>`:'').join('')}${q.other&&a.values.includes(q.other[0])?`<label for="${q.id}-other">${e(q.other[1])}</label><textarea class="notes" id="${q.id}-other" maxlength="1000">${e(a.otherText||'')}</textarea>`:''}</section>`;}
function render(){
...
function capture(){
 ...
 for(const q of questions.filter(q=>current.ids.includes(q.id))){
  ...
  if(current.reasons){
   a.reasons=Object.fromEntries((q.followups||[]).filter(o=>a.values.includes(o)).map(o=>[o,document.querySelector(`#${q.id}-reason-${q.options.indexOf(o)}`).value]));
